...
function UpdateItembuildsForHero() {
	var heroName = Game.GetPlayerInfo(Players.GetLocalPlayer()).player_selected_hero;
...
*/
	...
	for (var i in $('#ItembuildPanelsRoot').Children()) {
		var child = $('#ItembuildPanelsRoot').Children()[i]
		var te = child.GetChild(0);
		te.enabled = newState;
		if (te.BHasKeyFocus()) {
			$.GetContextPanel().SetFocus();
