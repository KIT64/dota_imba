...
function SearchItems() {
	...
	if (SearchingFor !== searchStr) {
		...
		$.Msg(ShopSearchOverlay.Children())
		for (var i in ShopSearchOverlay.Children()) {
			var child = ShopSearchOverlay.Children()[i]
...
//	Game.Events.F4Pressed.push(OpenCloseShop);
	GameUI.SetupVanillaKeyBinding("ShopToggle", OpenCloseShop)
	GameEvents.Subscribe('panorama_shop_open_close', OpenCloseShop);
//	Game.Events.F5Pressed.push(function() {
		GameUI.SetupVanillaKeyBinding("PurchaseQuickbuy", function() {
		if (QuickBuyTarget != null) {
			var bought = false;
